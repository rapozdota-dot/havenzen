...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($vehicleName === '' || $licensePlate === '') {
        ...
        if ($vehicleId > 0) {
            $stmt = $conn->prepare("
                UPDATE vehicles
                SET vehicle_name = ?, license_plate = ?, vehicle_type = ?, vehicle_color = ?, vehicle_model = ?, seat_capacity = ?, status = 'inactive'
                WHERE vehicle_id = ? AND driver_id = ?
            ");
            if ($stmt) {
                $stmt->bind_param('sssssiii', $vehicleName, $licensePlate, $vehicleType, $vehicleColor, $vehicleModel, $seatCapacity, $vehicleId, $driverUserId);
                $ok = $stmt->execute();
                $stmt->close();
            ...
            $stmt = $conn->prepare("
                INSERT INTO vehicles (vehicle_name, license_plate, vehicle_type, vehicle_color, vehicle_model, seat_capacity, driver_id, status)
                VALUES (?, ?, ?, ?, ?, ?, ?, 'inactive')
            ");
            if ($stmt) {
                $stmt->bind_param('sssssii', $vehicleName, $licensePlate, $vehicleType, $vehicleColor, $vehicleModel, $seatCapacity, $driverUserId);
                $ok = $stmt->execute();
                $stmt->close();
...
$modelValue = htmlspecialchars($vehicle['vehicle_model'] ?? '', ENT_QUOTES, 'UTF-8');
    ...
    <form method="POST" action="register_vehicle.php">
        <div class="form-grid">
            ...
            <div class="form-group">
                ...
                <input type="text" id="vehicle_model" name="vehicle_model" class="form-control" value="<?php echo $modelValue; ?>" placeholder="e.g., Toyota HiAce Commuter">
            </div>
            <div class="form-group">
